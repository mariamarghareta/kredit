...
class Transaksi extends CI_Model {
    ...
    public function cek_header_transaksi($kd_trans){
        ...
        $query = $this->db->select('t.*, c.*, b.nama_blok, b.kd_blok , ta.nomor_tanah, ta.kd_tanah , ifnull(bn.bayar,0) as balik_nama, bn.kd_nota as bn_kd_nota, kar.nama as bn_kar_nama, book.bayar as bayar_book, book.kd_nota as book_kd_nota, t.baliknama as cicilan_baliknama, t.biayabaliknama as besar_baliknama, agen.nama as nama_agen')
            ->from('transaksi t')
            ->join('customer c', 'c.kd_cust = t.kd_cust', 'left')
            ->join('tanah ta', 'ta.kd_tanah = t.kd_tanah', 'left')
            ->join('blok b', 'b.kd_blok = ta.kd_blok', 'left')
            ->join('balik_nama bn', 't.kd_trans = bn.kd_trans', 'left', 'left')
            ->join('karyawan kar','kar.kd_kar = bn.kd_kar', 'left', 'left')
            ->join('karyawan agen','agen.kd_kar = t.kd_agen', 'left', 'left')
            ->join('booking book','book.kd_trans = t.kd_trans', 'left', 'left')
    ...
    public function search_kdtrans($nama){
        $query = $this->db->select('t.kd_trans, cus.nama, b.nama_blok, tan.nomor_tanah')
            ->from('transaksi t')
            ->join('customer cus', 'cus.kd_cust = t.kd_cust', 'left')
            ->join('tanah tan', 'tan.kd_tanah = t.kd_tanah', 'left')
            ->join('blok b', 'b.kd_blok = tan.kd_blok', 'left')
            ->where('t.deleted', 0)
            ->group_start()
