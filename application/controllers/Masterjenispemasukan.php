...
class Masterjenispemasukan extends CI_Controller {
    ...
    public function show(){
        ...
        $this->load->view('masterjenispemasukan', $this->data);
    }
    public function add_new_data(){
        ...
        if ($this->form_validation->run() == TRUE){
            $hasil = $this->Jenispemasukan->insert($this->data['name']);
            if($hasil == 1){
                $this->clear();
                $this->data['msg'] = "<div id='err_msg' class='alert alert-success sldown' style='display:none;'>Jenis Pemasukan Berhasil ditambahkan</div>";
            }else{
                $this->data['msg'] = "<div id='err_msg' class='alert alert-danger sldown' style='display:none;'>Jenis Pemasukan Gagal ditambahkan</div>";
            }
        }
        $this->show();
    }
    ...
    public function update_data(){
        ...
        if($this->input->post('cancel')==false){
            if ($this->form_validation->run() == TRUE){
                $hasil = $this->Jenispemasukan->update($this->data['id'], $this->data['name']);
                if($hasil == 1){
                    $this->data['name']="";
                    $this->data['msg'] = "<div id='err_msg' class='alert alert-success sldown' style='display:none;'>Jenis Pemasukan Berhasil Terubah</div>";
                }else{
                    $this->data['msg'] = "<div id='err_msg' class='alert alert-danger sldown' style='display:none;'>Jenis Pemasukan Gagal Terubah</div>";
                }
            }
        }else{
            $this->data['name']="";
    ...
    public function delete_data(){
        ...
        $this->data['show_update']="slhide";
        $this->data['id'] = $this->input->post('id');
        $query = $this->Jenispemasukan->get_one_data($this->data['id']);
    ...
    public function confirm_del(){
        ...
        if($this->input->post('cancel') == true){
            ...
            if($this->Jenispemasukan->delete($this->data['id']) == 1){
